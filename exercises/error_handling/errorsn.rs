...
// Box means memory is on the heap, not the stack.
// dyn error::Error <-- dynamic abstract error
fn read_and_validate(b: &mut dyn io::BufRead) -> Result<PositiveNonzeroInteger, Box<dyn error::Error>> {
    ...
    // semi-colon or not?
    // semi-colon -> statement, not expression (does not return a value)
    // no semi-colon -> expression (returns a value)
    Ok(answer)
}
